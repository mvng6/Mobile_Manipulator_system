...
# Mobile Manipulator Docker 컨테이너 실행 스크립트
...
echo -e "${BLUE}🚀 Mobile Manipulator ROS Noetic 컨테이너를 실행합니다...${NC}"
echo -e "${BLUE}====================================================${NC}"
echo ""
...
# Docker 컨테이너 실행
docker run -it \
    --name mobile_manipulator_control_container \
    --hostname mobile-manipulator-docker \
    --network host \
    --privileged \
    -e DISPLAY=$DISPLAY \
    ...
    mobile_manipulator_ros_noetic:latest \
    bash -c "
